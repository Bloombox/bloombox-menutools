...
all: build
	@echo "bloombox-menutools is ready."
...
ifeq ($(BUILDBOT),yes)
test:
	wct --job-name "bloombox-menutools" --expanded --build-number "$(BUILD_NUMBER)" --sauce-tunnel-id "$(SAUCE_TUNNEL)" --plugin wct-jenkins
else
test:
	@wct --job-name "bloombox-menutools"
endif
